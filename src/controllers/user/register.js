        ...
        await database.User.create(newUser)
            ...
            await nodemailer
              ...
              .sendMail({
                ...
                html: `<h1 style="text-align:left; color:#00b9ee;">Welcome To Digital Career.Bangladesh</h1>
                      ...
                      <a style="text-decoration:none;color:#fff;font-size:15px;"href="${process.env.CLIENT_URL}/verifyEmail/${email_verify_token}">Activate Your Account</a>
                      </div>`
              })
              .then(async info => {
                await database.Activity.create({
                  activity_name: "Profile has been created succesfully.",
          ...
          .catch(err => {
            if (err.fields.username) {
              ...
              res.status(404).json({
                status: false,
                message: `Username ${existingUsername.slice(
                  1
                )} is already taken.`
              });
            } else {
              res.status(404).json({
                status: false,
                message: "Account creation faild"
              });
            }
          });
      });
